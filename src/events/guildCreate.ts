import { Events, ChannelType, PermissionFlagsBits, Client, Guild } from 'discord.js';
import { GuildConfig, IGuildConfig } from '../models/GuildConfig';

export async function setupGuildChannels(guild: Guild) {
    try {
        console.log(`Checking/Setting up channels for guild: ${guild.name}`);
        
        let config = await GuildConfig.findOne({ guildId: guild.id });
        if (config && config.clockInChannelId && config.clockOutChannelId && config.breakChannelId && config.reportsChannelId) {
            console.log(`Channels already exist for ${guild.name}`);
            return;
        }
